...
while aux!=1 and aux!=2:
	aux = int(input("\nVoce quer inserir Gama e Zc na forma polar (1) ou na retangular (2)?"))
	if aux==1:
		gama_pol[0] = float(input("Digite o valor do modulo de gama [1/km] : "))
		gama_pol[1] = float(input("Digite o valor do angulo de gama : "))
		zc_pol[0] = float(input("Digite o valor do modulo de Zc [ohm] : "))
		zc_pol[1] = float(input("Digite o valor do angulo de Zc : "))

		gama_rec = complex(math.cos(gama_pol[1]*(math.pi/180))*gama_pol[0],math.sin(gama_pol[1]*(math.pi/180))*gama_pol[0])
		zc_rec = complex(math.cos(zc_pol[1]*(math.pi/180))*zc_pol[0],math.sin(zc_pol[1]*(math.pi/180))*zc_pol[0])

	if aux==2:
		gama_rec = complex(input("Digite o valor de gama na forma retangular (X+Yj) [1/km] :"))
		raio = cmath.polar(gama_rec)
		angulo = raio[1]*(180/math.pi)
		gama_pol = [raio[0], angulo]

		zc_rec = complex(input("Digite o valor de Zc na forma retangular (X+Yj) [ohm] :"))
...
print("\n* O coeficiente de propagacao gama = sqrt(Zs*Ys) [1/km]\n//rec:", gama_rec, "// pol:", gama_pol,"//")
...
print("\n* A impedancia caracteristica Zc = sqrt(Zs/Ys) [ohm]\n// rec:", zc_rec, "// pol:", zc_pol,"//")
...
print("\n* A impedancia longituginal Zs = gama/Yc = Rs+jwLs [ohm/km]\n// rec:", z_rec, "// pol:", z_pol,"//")
print("\n* A admiancia transversal Ys = Zs/sqrt(Zc) = jwCs [S/km]\n// rec:", y_rec, "// pol:", y_pol,"//")
...
C_rec = yc_rec*cmath.sinh(gama_rec*l)
...
E_rec = cmath.cosh(gama_rec*l)
raio = cmath.polar(E_rec)
angulo = raio[1]*(180/math.pi)
E_pol = [raio[0], angulo]

F_rec = -zc_rec*cmath.sinh(gama_rec*l)
raio = cmath.polar(F_rec)
angulo = raio[1]*(180/math.pi)
F_pol = [raio[0], angulo]

G_rec = -yc_rec*cmath.sinh(gama_rec*l)
raio = cmath.polar(G_rec)
angulo = raio[1]*(180/math.pi)
G_pol = [raio[0], angulo]

H_rec =	cmath.cosh(gama_rec*l)
raio = cmath.polar(H_rec)
angulo = raio[1]*(180/math.pi)
H_pol = [raio[0], angulo]

print("\nA = D = cosh(gama*l) // B = Zc*sinh(gama*l) [ohm] // C = Yc*sinh(gama*l) [S]")
print("\nE = H = A // F = -B [ohm] // G = -C [S]")
